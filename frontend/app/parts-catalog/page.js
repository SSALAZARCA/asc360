...
export default function PartsCatalogPage() {
  ...
  // Same inline-edit mechanism already proven in Ajuste de Pedidos
  // (frontend/components/imports/AnalisisRepuestosTab.js's `editingDesc`/
  // `handleSaveDesc`), copied verbatim per explicit user request instead of
  // a bespoke per-row implementation. Editing a code that applies to
  // multiple vehicle models (multiple rows share one factory_part_number)
  // updates all of them together, by design -- same behavior as that
  // screen, not a bug here.
        ...
        <table className="master-table">
          ...
          <tbody>
            {loading ? (
              ...
              const rowKey = `${item.factory_part_number}-${item.section_code}-${i}`;
              return (
              <tr key={rowKey} className="hover:bg-white/5 transition-colors border-b border-white/5">
                ...
                <td style={{ maxWidth: '220px' }}>
                  {editingDesc?.fpn === item.factory_part_number ? (
                    <input
                      autoFocus={i === sortedItems.findIndex(x => x.factory_part_number === item.factory_part_number)}
                ...
                <td>
                  {(() => {
                    const fobVal = item.avg_fob_cost ?? item.preliminary_fob;
                    if (fobVal == null) return <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.68rem' }}>—</span>;
                    return (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '0.78rem', color: item.fob_is_preliminary ? '#fb923c' : '#38bdf8' }}>
                          ${Number(fobVal).toFixed(2)}
                        </span>
                        {item.fob_is_preliminary && (
                          <span style={{ fontSize: '0.52rem', fontWeight: 800, color: '#fb923c', letterSpacing: '0.08em', opacity: 0.8 }}>PREL</span>
                        )}
                      </div>
                    );
                  })()}
                ...
                <td>
                  {item.costo_importado != null
                    ? <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '0.78rem', color: item.fob_is_preliminary ? 'rgba(251,146,60,0.75)' : '#fb923c' }}>${Number(item.costo_importado).toLocaleString('es-CO', { maximumFractionDigits: 0 })}</span>
                        {item.fob_is_preliminary && <span style={{ fontSize: '0.52rem', fontWeight: 800, color: '#fb923c', letterSpacing: '0.08em', opacity: 0.8 }}>PREL</span>}
                      </div>
                ...
                <td>
                  {item.precio_distribuidor != null
                    ? <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '0.78rem', color: item.fob_is_preliminary ? 'rgba(250,204,21,0.65)' : '#facc15' }}>${Number(item.precio_distribuidor).toLocaleString('es-CO', { maximumFractionDigits: 0 })}</span>
                        {item.fob_is_preliminary && <span style={{ fontSize: '0.52rem', fontWeight: 800, color: '#fb923c', letterSpacing: '0.08em', opacity: 0.8 }}>PREL</span>}
                      </div>
                    : <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.68rem' }}>—</span>}
                </td>
                <td>
                  {item.precio_publico_calculado != null
                    ? <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '0.78rem', color: item.fob_is_preliminary ? 'rgba(74,222,128,0.55)' : '#4ade80' }}>${Number(item.precio_publico_calculado).toLocaleString('es-CO', { maximumFractionDigits: 0 })}</span>
                        {item.fob_is_preliminary && <span style={{ fontSize: '0.52rem', fontWeight: 800, color: '#fb923c', letterSpacing: '0.08em', opacity: 0.8 }}>PREL</span>}
                      </div>
                    : <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.68rem' }}>—</span>}
                </td>
