...
export default function InformeGerencialModal({ onClose }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `informe-gerencial-${desde}_${hasta}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
      onClose();
    } catch (err) {
      setError(err.message || 'Error generando el informe');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <div style={{
        ...
        <p style={{ fontSize: '11px', color: '#9ca3b8', marginBottom: '20px', lineHeight: '1.5' }}>
          Genera un PDF consolidado con KPIs gerenciales para el período seleccionado.
