...
specific `carga_archivo.log` is Fase 9.4's job, since only the caller knows
which load is asking; this module logs via the standard `logging` module,
which is the only log surface a clave-agnostic pure resolver can own).
"""
import datetime
import logging
import uuid
...
class TestResolverGenerico:
    async def test_returns_configured_value_when_a_vigente_row_exists(self):
        ...
        valor = await parametros.resolver(
    ...
    async def test_falls_back_to_the_caller_supplied_default_when_no_row_exists(self):
        ...
        valor = await parametros.resolver(db, "clave_inexistente", FECHA, default="valor_default")
...
class TestClavesDeFase2ConDefaultCodificado:
    ...
    async def test_tipos_inventario_incluidos_default(self):
        ...
        valor = await parametros.resolver_tipos_inventario_incluidos(db, FECHA)
    ...
    async def test_estados_backorder_vigentes_default(self):
        ...
        valor = await parametros.resolver_estados_backorder_vigentes(db, FECHA)
    ...
    async def test_dias_ventana_ingresos_default(self):
        ...
        valor = await parametros.resolver_dias_ventana_ingresos(db, FECHA)
    ...
    async def test_tolerancia_ingreso_pct_default(self):
        ...
        valor = await parametros.resolver_tolerancia_ingreso_pct(db, FECHA)
    ...
    async def test_crear_referencias_desconocidas_default(self):
        ...
        valor = await parametros.resolver_crear_referencias_desconocidas(db, FECHA)
