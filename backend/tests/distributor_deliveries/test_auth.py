"""
tests/distributor_deliveries/test_auth.py — Phase 3 role-guard for
`POST /distributor/deliveries`.

Mirrors `tests/historical_orders/test_auth.py`'s PR1 shape: a caller with
neither `parts_dealer` nor `superadmin` gets 403 before the DB is ever
...
Phase 3 stub, which returns 501 (the real service is wired in PR4).
"""
...
def test_distribuidor_reaches_the_stub():
    _override_db_only()
