...
def ensure_bucket_exists():
    try:
        ...
        logger.error(f"Error checking/creating MinIO bucket ({type(e).__name__})")
...
async def generate_and_upload_reception_pdf(
    ...
    try:
        ...
        logger.error(f"Error uploading PDF to MinIO ({type(e).__name__})")
        return ""
...
async def upload_file_to_minio(file_bytes: bytes, file_name: str, content_type: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error uploading file {file_name} to MinIO ({type(e).__name__})")
        return ""

async def get_pdf_stream_from_minio(object_name: str) -> Optional[bytes]:
    ...
    try:
        ...
        logger.error(f"Error extrayendo {object_name} de MinIO ({type(e).__name__})")
        return None


def fetch_reception_pdf_bytes(pdf_url: str) -> Optional[bytes]:
    """sdd/reception-email-notification Phase 3 (design ADR 1). Re-fetches
    the bytes of an already-uploaded reception PDF for the background
    email-attach path, given the presigned URL returned earlier by
    `generate_and_upload_reception_pdf`.

    Purely additive -- `generate_and_upload_reception_pdf` keeps its
    existing return type/behavior untouched (dozens of existing test
    monkeypatches depend on it staying a bare presigned-URL string).

    Only `urlparse(pdf_url).path` is trusted; the host portion is never
    used (the presigned URL's host may already be host-swapped for the
    bot's benefit, see `pdf_url` above). The bucket prefix is stripped to
    recover the MinIO object name. Never raises: empty/unparseable URL,
    a path that doesn't start with the bucket, or any MinIO error all
    return `None`."""
    if not pdf_url:
        return None
    ...
    if not path or not path.startswith(bucket_prefix):
        return None
    ...
    try:
        ...
        logger.error(f"Error fetching reception PDF bytes ({type(e).__name__})")
        return None
