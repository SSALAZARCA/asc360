...
async def search_orders_by_part_reference(
    q: str = "",
    db: AsyncSession = Depends(get_db),
    current_user: CurrentUser = Depends(get_current_user),
):
    """
    Search orders that contain spare parts matching the given reference.
    Returns one entry per order, with all matching parts grouped under it.
    """
    from app.models.order import OrderPart
    ...
    if not q or len(q.strip()) < 2:
        raise HTTPException(
            status_code=422,
            detail="Ingresá al menos 2 caracteres",
        )

    query_str = q.strip()

    stmt = (
        select(OrderPart, ServiceOrder, Vehicle, User)
        .join(ServiceOrder, OrderPart.order_id == ServiceOrder.id)
        .join(Vehicle, ServiceOrder.vehicle_id == Vehicle.id)
        .outerjoin(User, ServiceOrder.client_id == User.id)
        .where(OrderPart.reference.ilike(f"%{query_str}%"))
    )
    ...
    orders_map: dict = {}
    parts_map: dict = defaultdict(list)

    for part, order, vehicle, client in rows:
        order_id_str = str(order.id)
        if order_id_str not in orders_map:
            orders_map[order_id_str] = {
        ...
        parts_map[order_id_str].append({
            "reference": part.reference,
            "qty": part.qty,
            "status": part.status.value if part.status else None,
            "part_type": part.part_type.value if part.part_type else None,
        })

    return [
        {**order_data, "matched_parts": parts_map[oid]}
        for oid, order_data in orders_map.items()
    ]
