...
def _require_superadmin(current_user: CurrentUser = Depends(get_current_user)) -> CurrentUser:
...
async def create_limited_warranty(
    ...
    _: CurrentUser = Depends(_require_superadmin),
):
...
async def list_limited_warranties(
    db: AsyncSession = Depends(get_db),
